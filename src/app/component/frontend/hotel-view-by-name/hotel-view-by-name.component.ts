...
@Component({
  selector: 'app-hotel-view-by-name',
  templateUrl: './hotel-view-by-name.component.html',
  styleUrls: ['./hotel-view-by-name.component.css']
})
export class HotelViewByNameComponent {
  ...
  constructor(private _scripts: ScriptsMethodService, private _wishList: WishlistService, private _frontend: FrontendService, private route: ActivatedRoute, private router: Router, private renderer: Renderer2, private el: ElementRef,
    private geo: GeoService, private _meta: MetaService, private viewportScroller: ViewportScroller, private _location: Location){
  ...
  ngOnInit(){
    ...
    if (localStorage.getItem('search')) {
      this.params = JSON.parse(localStorage.getItem('search') as string);
      this.handleSearchParams(this.params);
    }else{
      //when search not exist then it will call this one
      const currentRoute = this.route.snapshot.url
      //console.log("const currentRoute = this.route.snapshot.url ", currentRoute)
      this.code = currentRoute[0].path;
      let nextDate = new Date();
      nextDate.setDate(nextDate.getDate() + 1);
      let checkout = moment(nextDate).format('YYYY-MM-DD');
      this.checkin = currentdate;
      this.checkout = checkout;
      this.params = {code:this.code, Name: '', checkin:this.checkin, checkout:this.checkout, adults:this.adults, child:this.child, num_rooms:this.num_rooms}
    ...
    //when url change by navigation
    let previousUrl = this._location.path();
    this._location.onUrlChange((url: string) => {
      // Check if the current URL is different from the previous URL
      //console.log("working on navigation ", url)
      if (url !== previousUrl) {
        //get hotel slug name from url
        let slug = url.substring(1)
        this.params.code = slug;
      ...
      // Update the previous URL with the current URL
      previousUrl = url;
    });
  }
  ...
  handleSearchParams(params: any) {
    ...
    if (this.checkin >= currentdate) {
      // Handle this case
      //console.log("working checkin ", this.checkin +">="+ currentdate);
    } else {
      // Get the next date
      ...
      this.checkout = checkout;
      // Update localStorage
      this.params = { code: this.code, checkin: this.checkin, checkout: this.checkout, adults: this.adults, child: this.child, num_rooms: this.num_rooms }
      this.getHotelListByHotel();
    }
  }
  ...
  getHotelListByHotel(){
    ...
    let queryData = {code: this.code, checkin: this.checkin, checkout:this.checkout , adults:this.adults, child:this.child, num_rooms:this.num_rooms}; //this.params;
    if(this.onSimilarHotel){
      queryData = {code: this.params.code, checkin: this.checkin, checkout:this.checkout , adults:this.adults, child:this.child, num_rooms:this.num_rooms};
      this.onSimilarHotel = false;
    }
    ...
    this._frontend.searchHotel(queryData).subscribe({
        next: (res:any)=>{
          ...
          //update hotel name in local storage
          //this.params.hotelName = this.listDetails.hotel_name;
          const changeStorageValue = { code: this.listDetails.slug, Name: this.listDetails.hotel_name, checkin: this.checkin, checkout: this.checkout, adults: this.adults, child: this.child, num_rooms: this.num_rooms }
          if(!localStorage.getItem("search")){
          ...
          console.log("hotel rooms",this.rooms);
          //
          this.loader = false;
        },
        error: err=>{
  ...
  onSimilarHotel: boolean = false;
  onChangeHotel(list:any){

      if(list.be_hotel_code){
        //get query params
        console.log("url ", list)
        this.params = {code:list.slug, Name: list.hotel_name, checkin:this.checkin, checkout:this.checkout, adults:this.adults, child:this.child, num_rooms:this.num_rooms};
        //call the search method
        //this.getHotelListByHotel();
        this.onSimilarHotel = true;
        //set item in local storage for updated url
  ...
  setMap_Lat_Long(){
    if(this.listDetails){
      ...
      this.center = {
        lat: parseFloat(this.listDetails?.hotel_lat),
        lng: parseFloat(this.listDetails?.hotel_long)
      };
    }
  }
